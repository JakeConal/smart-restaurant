...
function FavoritesContent() {
  ...
  useEffect(() => {
    const fetchFavoriteItems = async () => {
      ...
      try {
        const response = await menuApi.getMenu(currentToken || '', { limit: 100 }) as MenuResponse;
        const favoriteItems = response.menu.items.filter((item: MenuItem) => favorites.includes(item.id));
  ...
  const getItemPhoto = (item: MenuItem) => {
    if (item.primaryPhotoId) {
      return menuApi.getItemPhoto(item.primaryPhotoId);
    }
    return null;
  };
      ...
      <div className="px-6">
        {loading ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="text-gray-500 mb-6">Start adding items to your favorites while browsing the menu</p>
            <button
          ...
          <div className="grid grid-cols-2 gap-4">
            {items.map((item) => (
              ...
              >
                ...
                <button
                  onClick={() => router.push(`/menu/${item.id}?token=${currentToken}`)}
                ...
                >
                  ...
                  <div className="p-3">
                    ...
                    <p className="text-[#fa4a0c] font-bold mt-1">${item.price.toFixed(2)}</p>
                  </div>
                </button>
              </div>
            ))}
